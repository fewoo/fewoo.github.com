a = function(opt) {
    var scroll = function(o) {
            ...
            this.resize(this.step || this.moveChild.eq(0).outerWidth());
            if (this.autoTime) {
                var obj = this;
    ...
    $.extend(scroll.prototype, {
        ...
        touchmove: function(e) {
            ...
            if (Math.abs(x) - Math.abs(y) > this.min) {
                ...
                this.moveDom.css({
                    ...
                    "-webkit-transform": this.enableTransX ? "translateX(" + ((this.index - 1) * -100) + "%)" : "translate3D(" + offset + "px,0,0)",
                    "-webkit-transition": "0"
                });
            } else {
                if (!this.lockScrY)
        ...
        stepMove: function(no, isSetOffsetIndex) {
            ...
            this.moveDom.css({
                "-webkit-transform": this.enableTransX ? "translateX(" + ((this.index - 1) * -100) + "%)" : "translate3D(" + tran + "px,0,0)",
...
$(function(){
    var moveDom=$("#loopImgUl"),
        moveChild=$("#loopImgUl").children("li"),
        tab=$("#loopImgBar>li");

    var b=a({moveDom:moveDom,moveChild:moveChild,tab:tab,loopScroll:true,loadImg:true});
})